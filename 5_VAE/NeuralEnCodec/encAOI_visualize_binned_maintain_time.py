...
if __name__ == "__main__":
    ...
    model_path = 'best_model_binned_neural_maintain_time.pth'  # Updated model path
    data_path = '/mnt/e/neuro2voc/neural_encodec/model_outputs_binned_maintain_time'
    ...
    # Process 5 input samples
    for i in range(1, 6):
        input_file = f'input_{i}.pt'
        output_file = f'output_{i}.pt'
        
        # Load input data
        input_data = torch.load(os.path.join(data_path, input_file), weights_only=True)
        input_data = input_data.to(device)
        
        # Convert model parameters and input to same dtype
        input_data = input_data.to(torch.float32)
        model = model.to(torch.float32)
        
        # Generate output
        with torch.no_grad():
            encoded, scale = model.encode(input_data)
            output = model.decode(encoded, scale)
            
        # Save output
        torch.save(output.cpu(), os.path.join(data_path, output_file))
