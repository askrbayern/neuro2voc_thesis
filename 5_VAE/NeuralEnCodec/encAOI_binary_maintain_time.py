...
class NeuralSEANetDecoder(nn.Module):
    def __init__(self, channels=75, dimension=75, depth=2, norm='time_group_norm'):
        ...
        # Changed to maintain time dimension
        self.layers = nn.ModuleList([
            nn.Sequential(
                nn.LSTM(dimension, dimension, depth, batch_first=True),
                nn.GroupNorm(1, dimension) if norm == 'time_group_norm' else nn.Identity(),
                ...
                # Conv1d
                nn.Conv1d(
                    dimension, dimension, kernel_size=3, stride=1, padding=1
                )
            ) for _ in range(2)
        ])
        ...
        self.conv_out = nn.Conv1d(
            dimension, channels, kernel_size=3, stride=1, padding=1
        )
...
#     num_epochs = 200
#     learning_rate = 3e-4
#     # folder_name = '/mnt/m/neuro2voc/task-4/'
#     # dataset_name = 'AOI_dataset_750ms_pre_400ms_segment_length_concatenated'

#     folder_name = '/mnt/m/neuro2voc/task-5/'
#     # dataset_name = 'AOI_dataset_400ms_pre_600ms_segment_length_1_3_overlap'
#     # dataset_name = 'AOI_dataset_1000ms_pre_300ms_segment_length_0.33_overlap'
#     # dataset_name = 'AOI_50ms_pre_300ms_segment_length_end_bin/data_tensor'
#     # dataset_name = 'DTW_AOI_250hz_50ms_pre_50ms_endEarly_bin/data'
#     dataset_name = 'AOI_30kHz_60ms_pre_30ms_segment_overlap_0.33'
