...
class AOISegmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.load(os.path.join(self.data_dir, self.data_files[idx]), weights_only=True)
...
class NeuralSEANetEncoder(nn.Module):
    ...
    def forward(self, x):
        # print(f"Input shape: {x.shape}")  # Add this line
        x = self.conv1(x)
        # print(f"After first conv shape: {x.shape}")  # Add this line
        x = self.norm1(x)
...
class NeuralSEANetDecoder(nn.Module):
    def __init__(self, channels=75, dimension=75, depth=2, norm='time_group_norm'):
        ...
        # Reverse of encoder layers with adjusted last ConvTranspose1d
        self.layers = nn.ModuleList([
            nn.Sequential(
                nn.LSTM(dimension, dimension, depth, batch_first=True),
                nn.GroupNorm(1, dimension) if norm == 'time_group_norm' else nn.Identity(),
                nn.ReLU(),
                nn.ConvTranspose1d(
                    dimension, dimension, kernel_size=8, stride=4, padding=2, output_padding=0
                )
            ) for _ in range(2)
        ] + [
            nn.Sequential(
                ...
                nn.ReLU(),
                nn.ConvTranspose1d(
                    dimension, dimension, kernel_size=9, stride=4, padding=2, output_padding=3
                )
            )
        ])
        ...
        self.conv_out = nn.ConvTranspose1d(
            dimension, channels, kernel_size=7, stride=1, padding=3, output_padding=0
        )

    def forward(self, x):
        for idx, layer in enumerate(self.layers):
            lstm, norm, relu, deconv = layer
            # print(f"Decoder Layer {idx+1} input shape: {x.shape}")
            x = x.permute(0, 2, 1)  # (batch, time, channels)
            x, _ = lstm(x)
            x = x.permute(0, 2, 1)  # (batch, channels, time)
            x = norm(x)
            x = relu(x)
            x = deconv(x)
            # print(f"Decoder Layer {idx+1} output shape: {x.shape}")
        x = self.norm1(x)
        x = self.conv_out(x)
...
def test_trained_model(model_path, data_path):
    ...
    print("\nLoading and processing 5 data samples...")
    try:
        dataset = AOISegmentDataset(data_path)
        if len(dataset) < 5:
            print("Dataset has fewer than 5 samples!")
            return
            
        # Process 5 samples
        for i in range(5):
            x = dataset[i].unsqueeze(0).to(device).bfloat16()
            ...
            with torch.no_grad():
                ...
                print(f"Scale shape: {scale.shape if scale is not None else 'None'}")
                ...
                torch.save(x.cpu(), os.path.join(save_dir, f"input_{i+1}.pt"))
                torch.save(decoded.cpu(), os.path.join(save_dir, f"output_{i+1}.pt"))
                print(f"Saved input_{i+1}.pt and output_{i+1}.pt")
...
if __name__ == "__main__":
    ...
    data_path = '/mnt/m/neuro2voc/task-5/AOI_30kHz_60ms_pre_30ms_segment_overlap_0.33'
    
    test_trained_model(model_path, data_path)
